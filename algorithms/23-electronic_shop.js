...
function getMoneySpent(keyboards, drives, b) {
    ...
    // Get All items less than the budget
    const myKeyboards =  keyboards.filter( num => num<=b);
    const myDrives = drives.filter(num => num<=b);
    
    
    for(let i = 0 ;i<myKeyboards.length;i++){
        for(let j = 0;j<myDrives.length;j++ ){
            let sum = myKeyboards[i]+myDrives[j];
            if(sum <= b && sum >max){
                max = sum;
            }
        }
